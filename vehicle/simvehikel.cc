...
void vehikel_basis_t::verlasse_feld()
{
	...
	if(!get_flag(not_on_map)  &&  (gr==NULL  ||  !gr->obj_remove(this)) ) {
		...
		if(welt->is_in_map_limits( get_pos().get_2d() )) {
...
DBG_MESSAGE("vehicle_t::rdwr_from_convoi()","bought at %i/%i.",(insta_zeit%12)+1,insta_zeit/12);
	...
	if(file->is_saving()) {
		...
		for(int i=0; i<fracht_count; i++) {
			...
			if(  (besch==NULL  ||  ware.menge>0)  &&  welt->is_in_map_limits(ware.get_zielpos())  ) {	// also add, of the besch is unknown to find matching replacement
				fracht.insert(ware);
...
//DBG_MESSAGE("aircraft_t::calc_route()","search runway start near %i,%i,%i with corner in %x",start.x,start.y,start.z, approach_dir);
	...
	if(!start_in_the_air) {
		...
		if(start_dir!=koord(0,0)) {
			...
			do {
				if(!welt->is_in_map_limits(search_start.get_2d()+(start_dir*endi)) ) {
					break;
				}
...
//DBG_MESSAGE("aircraft_t::calc_route()","take off ok");
	...
	if(!end_in_air) {
		...
		if(end_dir!=koord(0,0)) {
			...
			// now add all runway + 3 ...
			do {
				if(!welt->is_in_map_limits(search_end.get_2d()+(end_dir*endi)) ) {
					break;
				}
	...
	if(!end_in_air) {
		...
		for(  int  i=0;  i<16;  i++  ) {
			...
			if(welt->is_in_map_limits(circlepos)) {
