				...
				if(over) {
					if(!over->is_overtaking()) {
						...
						if(  road_vehicle_t const* const car = obj_cast<road_vehicle_t>(dt)  ) {
							...
							if(  ocnv  ) {
								if(  next_lane==0  &&  !is_overtaking()  &&  !other_lane_blocked(false)  &&  !ocnv->is_overtaking()   &&  can_overtake( ocnv, (ocnv->get_state()==convoi_t::LOADING ? 0 : ocnv->get_akt_speed()), ocnv->get_length_in_steps()+ocnv->get_vehikel(0)->get_steps())  ) {
									if(current_speed==0) {
										ms_traffic_jam = 0;
										current_speed = 48;
									}
							...
							if(  caut  ) {
								if(  next_lane==0  &&  !is_overtaking()  &&  !other_lane_blocked(false)  &&  !caut->is_overtaking()  &&  can_overtake(caut, caut->get_current_speed(), VEHICLE_STEPS_PER_TILE)  ) {
									if(current_speed==0) {
										ms_traffic_jam = 0;
										current_speed = 48;
									}
