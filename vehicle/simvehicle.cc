...
bool road_vehicle_t::choose_route( sint32 &restart_speed, ribi_t::ribi richtung, uint16 index )
{
	...
	if(  target_halt.is_bound()  ) {
		...
		if(  can_go_there  ) {
			...
			if(  !target_rt.find_route(welt, next3d, this, speed_to_kmh(cnv->get_min_top_speed()), richtung, cnv->get_highest_axle_load(), cnv->get_tile_length(), cnv->get_weight_summary().weight / 1000, 33 )  ) {
...
rail_vehicle_t::~rail_vehicle_t()
{
	if (cnv && leading) {
		...
		if (!r.empty() && route_index < r.get_count()) {
			...
			uint16 dummy;
			block_reserver(&r, cnv->back()->get_route_index(), dummy, target_halt.is_bound() ? 100000 : 1, false, false);
		}
	}
	grund_t *gr = welt->lookup(get_pos());
	if(gr) {
		...
		if(leading) {
			if(cnv!=NULL  &&  cnv!=(convoi_t *)1) {
				...
				if(  !r.empty()  &&  route_index + 1U < r.get_count() - 1  ) {
					...
					uint16 dummy;
					block_reserver(&r, cnv->back()->get_route_index(), dummy, 100000, false, false);
					target_halt = halthandle_t();
				}
			}
				...
				if(  !check_for_finish  &&  c->get_state()!=convoi_t::SELF_DESTRUCT  &&  (c->get_state()==convoi_t::DRIVING  ||  c->get_state()>=convoi_t::LEAVING_DEPOT)  ) {
					...
					uint16 next_signal;
					cnv = c;
					if(  block_reserver(&r, max(route_index,1)-1, next_signal, num_index, true, false)  ) {
...
sint32 rail_vehicle_t::is_longblock_signal_clear( signal_t *sig, uint16 next_block, sint32 &restart_speed )
{
	...
	uint16 next_signal; 
	bool success = block_reserver(cnv->get_route(), next_block + 1, next_signal, 0, true, false);
	if(!success)
	{
	...
	while(fahrplan_index != cnv->get_schedule()->get_aktuell()) 
	{
		...
		if(success) 
		{
			success = block_reserver(&target_rt, 1, next_next_signal, 0, true, false);
		}

		if(success && next_next_signal < INVALID_INDEX) 
		{
...
sint32 rail_vehicle_t::activate_choose_signal(const uint16 start_block)
{
	...
	else
	{
		...
		blocks = block_reserver(cnv->get_route(), start_block, next_signal, 100000, true, false, true);
		if(!blocks) 
		{
...
sint32 rail_vehicle_t::is_pre_signal_clear( signal_t *sig, uint16 next_block, sint32 &restart_speed )
{
	...
	if( sint32 blocks = block_reserver( cnv->get_route(), next_block+1, next_signal, 0, true, false )  ) {
		if(  next_signal == INVALID_INDEX  ||  cnv->get_route()->position_bei(next_signal) == cnv->get_route()->back()  ||  is_signal_clear( next_signal, sig, restart_speed )  ) {
...
sint32 rail_vehicle_t::is_signal_clear(uint16 next_block, signal_t* sig, sint32 &restart_speed)
{
	if(  sig==NULL  ) {
			...
			if(!weg_frei)
			{
				...
				else
				{
					...
					else if(working_method == absolute_block || working_method == track_circuit_block)
					{
						...
						if(next_signal - route_index <= sighting_distance_tiles)
						{
							weg_frei = block_reserver( &route, last_index, next_signal, 0, true, false );
						}
					}
 ...
 * @author prissi
 */
sint32 rail_vehicle_t::block_reserver(route_t *route, uint16 start_index, uint16 &next_signal_index, int count, bool reserve, bool force_unreserve, bool is_choosing)
{
	bool success = true;
#ifdef MAX_CHOOSE_BLOCK_TILES
	...
	uint16 end_marker_index = INVALID_INDEX;
	...
	for( ; success && count >= 0 && i < route->get_count(); i++)
	{
		...
		if(reserve)
		{
			...
			if(!sch1->reserve(cnv->self, ribi_typ(route->position_bei(max(1u,i)-1u), route->position_bei(min(route->get_count()-1u,i+1u))))) 
			{
				...
				if(next_signal_working_method == drive_by_sight || next_signal_working_method == moving_block)
				{
				...
				if(next_signal_working_method == track_circuit_block && last_stop_signal_index != INVALID_INDEX)
				{
					next_signal_index = last_stop_signal_index;
					break;
				}
	...
	if(!success || ((next_signal_index < INVALID_INDEX) && (next_signal_working_method == absolute_block || next_signal_working_method == track_circuit_block || next_signal_working_method == cab_signalling)))
	{
		...
		for(uint32 j = relevant_index + 1; j < i; j++)
		{
			if(i != skip_index)
			{
		...
		if(last_choose_signal_index < INVALID_INDEX && !is_choosing && not_entirely_free)
		{
			...
			return activate_choose_signal(last_choose_signal_index); 
		}
					...
					if(count >= 0 || signal->get_pos() != route->position_bei(next_signal_index))
					{
						...
						case 4:
							...
							else if(counter + add_value >= 1)
							{
								...
								{
									signal->set_state(use_no_choose_aspect ? roadsign_t::preliminary_caution_no_choose : roadsign_t::preliminary_caution);
								}	
							}
							else if(next_signal_index == INVALID_INDEX || signal->get_pos() != route->position_bei(next_signal_index))
							{
						...
						case 5:
							...
							else if(counter + add_value >= 2)
							{
								...
								{
									signal->set_state(use_no_choose_aspect ? roadsign_t::advance_caution_no_choose : roadsign_t::advance_caution);
								}
							}
							else if(counter + add_value >= 1)
							{
