...
sint64 vehicle_t::calc_revenue(const koord3d& start, const koord3d& end) const
{
	...
	FOR(slist_tpl<ware_t>, const& ware, fracht) {
		...
		if ( welt->get_settings().is_overloading_revenue_reduced() ) {
			price *= ((get_total_cargo() > get_cargo_max()) && (get_cargo_max() > 0)) ? (sint64)get_total_cargo()/(sint64)get_cargo_max() : (sint64)1;
		}
