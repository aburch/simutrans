		...
		if (!fracht.empty())
		{
			for(  slist_tpl<ware_t>::iterator i = fracht.begin(), end = fracht.end();  i != end;  ) {
				...
				if(  !end_halt.is_bound() || !via_halt.is_bound()  ) {
					...
					if(halt != end_halt && welt->get_settings().is_avoid_overcrowding() && tmp.is_passenger() && !halt->is_within_walking_distance_of(via_halt) && halt->is_overcrowded(tmp.get_besch()->get_index()))
					{
...
bool vehicle_t::reroute(const uint16 reroute_index, const koord3d &ziel, route_t* route)
{
	...
	bool done = route && calc_route(route->position_bei(reroute_index), ziel, speed_to_kmh(cnv->get_min_top_speed()), cnv->has_tall_vehicles(), &xroute);
	if(done && live)
	{
	...
	if(  leading  ) {
		...
		if(str->has_sign()) {
			...
			if(rs) {
				...
				if(rs->get_besch()->is_traffic_light()  &&  (rs->get_dir()&direction90)==0) {
					// wait here
					restart_speed = 16;
					return false;
				}
...
sint32 rail_vehicle_t::activate_choose_signal(const uint16 start_block, uint16 &next_signal_index, uint32 brake_steps, uint16 modified_sighting_distance_tiles, route_t* route, sint32 modified_route_index)
{
	...
	for(uint32 idx = break_index; choose_ok && idx < route->get_count(); idx++)
	{
		...
		if(way->has_signal())
		{
			signal_t *sig = gr->find<signal_t>(1);
 			ribi_t::ribi ribi = ribi_type(route->position_bei(max(1u, modified_route_index) - 1u));	
	...
	else
	{
		...
		can_find_route = target_rt.calc_route(welt, route->position_bei(start_block), target->get_pos(), this, speed_to_kmh(cnv->get_min_top_speed()), cnv->get_highest_axle_load(), cnv->has_tall_vehicles(), cnv->get_tile_length(), SINT64_MAX_VALUE, cnv->get_weight_summary().weight / 1000); 
...
bool rail_vehicle_t::can_enter_tile(const grund_t *gr, sint32 &restart_speed, uint8)
{
	...
	ribi_t::ribi ribi = ribi_type(cnv->get_route()->position_bei(max(1u, route_index) - 1u), cnv->get_route()->position_bei(min(cnv->get_route()->get_count() - 1u, route_index + 1u)));
	...
	if(working_method == absolute_block || working_method == track_circuit_block || working_method == drive_by_sight || working_method == token_block || working_method == time_interval || working_method == time_interval_with_telegraph)
	{
		...
		for(uint16 tiles_to_check = 1; tiles_to_check <= modified_sighting_distance_tiles; tiles_to_check++)
		{
			...
			ribi_t::ribi ribi = ribi_type(cnv->get_route()->position_bei(max(1u,modified_route_index)-1u), cnv->get_route()->position_bei(min(cnv->get_route()->get_count()-1u,modified_route_index+1u)));
	...
	{	
		...
		ribi_t::ribi ribi = ribi_type(cnv->get_route()->position_bei(max(1u, check_route_index) - 1u), cnv->get_route()->position_bei(min(max_element, check_route_index + 1u)));
	...
	if(route_steps <= brake_steps || brake_steps < 0)
	{
		...
		{
			const uint16 check_route_index = next_block <= 0 ? 0 : next_block - 1u;
			ribi_t::ribi ribi = ribi_type(cnv->get_route()->position_bei(max(1u, (min(max_element, check_route_index))) - 1u), cnv->get_route()->position_bei(min(max_element, check_route_index + 1u)));
	...
	{
		if(working_method != one_train_staff || welt->lookup(w_current->get_pos())->find<signal_t>()->get_besch()->get_working_method() == one_train_staff)
		{
...
sint32 rail_vehicle_t::block_reserver(route_t *route, uint16 start_index, uint16 modified_sighting_distance_tiles, uint16 &next_signal_index, int count, bool reserve, bool force_unreserve, bool is_choosing, bool is_from_token, bool is_from_starter, bool is_from_directional, uint32 brake_steps)
{
	...
	for( ; success && count >= 0 && i < route->get_count(); i++)
	{
		...
		if(reserve)
		{
			...
			roadsign_t* rs = gr->find<roadsign_t>();
			ribi_t::ribi ribi = ribi_type(route->position_bei(max(1u,i)-1u), route->position_bei(min(route->get_count()-1u,i+1u)));
			...
			{			 
				...
				else
				{
					...
					if(signal && signal->get_besch()->is_longblock_signal() && check_halt.is_bound() && (signal->get_besch()->get_working_method() == time_interval || signal->get_besch()->get_working_method() == time_interval_with_telegraph || signal->get_besch()->get_working_method() == absolute_block))
					{
						signal = NULL;
					}
				}
				...
				if(signal && (signal != first_station_signal || !set_first_station_signal))
				{
					...
					if(!signal->get_besch()->is_pre_signal()) // Stop signal or multiple aspect signal
					{
						...
						if(first_stop_signal_index >= INVALID_INDEX)
						{
							...
							if(next_signal_working_method == time_interval || next_signal_working_method == time_interval_with_telegraph)
							{
								...
								else // Junction signal
								{
									...
									if(signal->get_besch()->is_longblock_signal() && (station_signal || next_signal_working_method == time_interval_with_telegraph))
									{
						...
						{
							if(signal->get_besch()->is_longblock_signal() && (last_longblock_signal_index < INVALID_INDEX || working_method == time_interval_with_telegraph))
							{
						...
						if(signal->get_besch()->is_longblock_signal() || (station_signal && last_longblock_signal_index == INVALID_INDEX))
						{
			...
			if(attempt_reservation && !sch1->reserve(cnv->self, ribi_type(route->position_bei(max(1u,i)-1u), route->position_bei(min(route->get_count()-1u,i+1u))), rt, (working_method == time_interval || working_method == time_interval_with_telegraph)))
			{
				...
				if((next_signal_working_method == track_circuit_block
					...
					|| sch1->can_reserve(cnv->self, ribi_type(route->position_bei(max(1u,i)-1u), route->position_bei(min(route->get_count()-1u,i+1u))), schiene_t::directional)))
				{
					next_signal_index = last_stop_signal_index;
					break;
				}
		...
		else if(sch1) // Unreserve
		{
			...
			if(sch1->has_signal()) 
			{
				ribi_t::ribi direction_of_travel = ribi_type(route->position_bei(max(1u,i)-1u), route->position_bei(min(route->get_count()-1u,i+1u)));
		...
		if(no_reverse || working_method == one_train_staff)
		{
			do
			{
				...
				if(token_block_blocks && next_next_signal < INVALID_INDEX) 
				{
					...
					{
						if(signal_t* sg = welt->lookup(route->position_bei(next_next_signal))->get_weg(get_waytype())->get_signal(ribi_type((route->position_bei(next_next_signal - 1)), route->position_bei(next_next_signal))))
						{
	...
	if(!success || !directional_reservation_succeeded || ((next_signal_index < INVALID_INDEX) && (next_signal_working_method == absolute_block || next_signal_working_method == track_circuit_block || next_signal_working_method == cab_signalling || ((next_signal_working_method == time_interval || next_signal_working_method == time_interval_with_telegraph) && !next_signal_protects_no_junctions))))
	{
		...
		ribi_t::ribi direction = ribi_type(route->position_bei(max(1u,start_index)-1u), route->position_bei(min(route->get_count()-1u,start_index+1u)));
			...
			if(sch0) 
			{
				...
				{		
					...
					if(sig) 
					{
						...
						if(sig->get_no_junctions_to_next_signal() && !sig->get_besch()->is_pre_signal() && (sig->get_besch()->get_working_method() == time_interval || sig->get_besch()->get_working_method() == time_interval_with_telegraph))
						{
						...
						if(route && !sig->get_besch()->is_pre_signal() && (w && (w->get_working_method() == absolute_block || w->get_working_method() == token_block || w->get_working_method() == time_interval || w->get_working_method() == time_interval_with_telegraph)))
						{
							...
							for(int i = min(route_index, route->get_count() - 1); i > 0; i--)
							{
								...
								{
									ribi = ribi_type(route->position_bei(i - 1), current_pos);
								}
				...
				halthandle_t last_tile_halt = cnv && route_index - 2 < cnv->get_route()->get_count() ? haltestelle_t::get_halt(cnv->get_route()->position_bei(route_index - 2), get_owner()) : halthandle_t(); 
				...
				{
					ribi_t::ribi direction = ribi_type(cnv->get_route()->position_bei(0u), cnv->get_route()->position_bei(1u));
...
//DBG_MESSAGE("air_vehicle_t::find_route_to_stop_position()","some free: find route from index %i",search_for_stop);
		...
		if(!target_rt.find_route( welt, rt->position_bei(search_for_stop), this, 500, ribi_t::all, cnv->get_highest_axle_load(), cnv->get_tile_length(), cnv->get_weight_summary().weight / 1000, 100, cnv->has_tall_vehicles())) {
