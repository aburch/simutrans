...
//DBG_MESSAGE("air_vehicle_t::can_enter_tile()","index %i<>%i",route_index,touchdown);
	...
	if(  route_index == touchdown - HOLDING_PATTERN_OFFSET - HOLDING_PATTERN_LENGTH &&  state != circling  )
	{
		...
		const int runway_state = block_reserver( touchdown + SET_LANDING_OFFSET - HOLDING_PATTERN_OFFSET, search_for_stop+1, true ); // LOOKS OKAY!
		if(runway_state == 1)
		{
			route_index += HOLDING_PATTERN_LENGTH;
...
void air_vehicle_t::hop(grund_t* gr)
{
	...
	switch(state) {
		...
		case landing: {
			...
			if (route_index >= touchdown - SET_LANDING_OFFSET)  { //modified. "too short"
				// come down, now!
