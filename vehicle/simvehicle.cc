						...
						// check, if we reached a choose point
							...
							if(  target_halt.is_bound()  ) {
								...
								if(  can_go_there  ) {
									...
									if(  !target_rt.find_route( welt, next3d, this, speed_to_kmh(cnv->get_min_top_speed()), curr_90direction, cnv->get_highest_axle_load(), cnv->get_weight_summary().weight / 1000, 33 )  ) {
										// nothing empty or not route with less than 33 tiles
										target_halt = halthandle_t();
										restart_speed = 0;
										return false;
...
bool rail_vehicle_t::can_enter_tile(const grund_t *gr, int & restart_speed, bool)
{
		...
		if(!weg_frei)
		{
			...
			if(!weg_frei)
			{
				...
				else
				{
					...
					else if(working_method == absolute_block)
					{
						// With absolute block signalling, it is necessary to be within the sighting distance or use a distant (pre-signal).
	...
	if(working_method == absolute_block)
	{
...
bool rail_vehicle_t::block_reserver(route_t *route, uint16 start_index, uint16 &next_signal_index, int count, bool reserve, bool force_unreserve)
{
		...
		if(reserve)
		{
			...
			if(sch1->has_signal())
			{
				...
				if(signal)
				{
					...
					{
						if(count || pre_signals.get_count())
						{
							signs.append(gr);
						}
	...
	if(pre_signals.get_count() == 1 && working_method == absolute_block && end_marker_index == INVALID_INDEX)
	{
	...
	if(next_signal_index != INVALID_INDEX && (working_method == absolute_block || working_method == cab_signalling || working_method == token_block))
	{
...
//DBG_MESSAGE("aircraft_t::find_route_to_stop_position()","can approach? (cnv %i)",cnv->self.get_id());
	...
	if(target==NULL  ||  !target->hat_weg(air_wt)) {
		target_halt = halthandle_t();
		DBG_MESSAGE("aircraft_t::find_route_to_stop_position()","no runway found at (%s)",rt->position_bei(search_for_stop).get_str());
...
//	DBG_MESSAGE("aircraft_t::find_route_to_stop_position()","state %i",state);
	if(!target_halt->find_free_position(air_wt,cnv->self,obj_t::air_vehicle)  ) {
		...
		// now search a route
//DBG_MESSAGE("aircraft_t::find_route_to_stop_position()","some free: find route from index %i",search_for_stop);
		route_t target_rt;
		flight_state prev_state = state;
		state = looking_for_parking;
		if(!target_rt.find_route( welt, rt->position_bei(search_for_stop), this, 500, ribi_t::alle, cnv->get_highest_axle_load(), cnv->get_weight_summary().weight / 1000, 100 )) {
...
//DBG_MESSAGE("aircraft_t::calc_route()","take off ok");
	...
	//DBG_MESSAGE("aircraft_t::calc_route_internal()","departing=%i  touchdown=%i   search_for_stop=%i   total=%i  state=%i",takeoff, touchdown, search_for_stop, route.get_count()-1, state );
	return true;
}
