...
sint32 convoy_t::calc_speed_holding_force(sint32 speed /* in m/s */, sint32 Frs /* in N */)
{
	return min((adverse.cf * (speed * speed)).integer(), get_force(speed) - Frs); /* in N */
}
	...
	//{
		...
		while (delta_t > 0)
		{
			...
			if (delta_t >= DT_SLICE && (sint32)abs(df.integer()) > weight.weight / (10 * DT_SLICE_SECONDS))
			{
				...
				//count2++;
				v += (df * fraction_t(DT_SLICE_SECONDS, weight.weight)).integer();
				dt = DT_SLICE;
			}
			else
			{
