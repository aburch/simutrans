...
bool interaction_t::process_event(event_t &ev)
{
	if (ev.ev_class == EVENT_SYSTEM  &&  ev.ev_code == SYSTEM_QUIT) {
		...
		if (env_t::server  &&  env_t::server_save_game_on_quit) {
			...
			loadsave_t file;
			if (file.wr_open(fn, loadsave_t::zipped, "hashes", SAVEGAME_VER_NR, EXPERIMENTAL_VER_NR, EXPERIMENTAL_REVISION_NR)) {
			...
			world->save(pak_name.c_str(), loadsave_t::autosave_mode, SERVER_SAVEGAME_VER_NR, EXPERIMENTAL_VER_NR, EXPERIMENTAL_REVISION_NR, false);
			env_t::restore_UI = old_restore_UI;
		}
