...
wget --post-data "version=0&choice=all&submit=Export!" --delete-after http://www.simutrans-germany.com/translator/script/wrap.php || {
  echo "Error: generating file language_pack-Base+texts.zip failed (wget returned $?)" >&2;
  exit 3;
}
wget -N http://www.simutrans-germany.com/translator/data/tab/language_pack-Base+texts.zip || {
