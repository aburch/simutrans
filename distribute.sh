...
wget --post-data "version=4&choice=all&submit=Export!" --delete-after http://translator.simutrans.com/script/wrap.php || {
  echo "Error: generating file language_pack-Base+texts.zip failed (wget returned $?)" >&2;
  exit 3;
}
wget -N http://translator.simutrans.com/tmp/language_pack-Base+texts.zip || {
