...
crossing_t::crossing_t(player_t* const player, koord3d const pos, kreuzung_desc_t const* const desc, uint8 const ns) : 
#ifdef INLINE_OBJ_TYPE
...
void crossing_t::finish_rd()
{
	...
	if(gr==NULL  ||  !gr->hat_weg(desc->get_waytype(0))  ||  !gr->hat_weg(desc->get_waytype(1))) {
		...
		const kreuzung_desc_t *test = crossing_logic_t::get_crossing( desc->get_waytype(0), desc->get_waytype(1), w1->get_desc()->get_topspeed(), w2->get_desc()->get_topspeed(), welt->get_timeline_year_month());
		if (test  &&  test!=desc) {
			desc = test;
		}
