...
bool classify_file_data(rdwr_stream_t *stream, file_info_t *info)
{
	...
	if (strstart(buf, SAVEGAME_PREFIX)) {
		info->version = loadsave_t::int_version(buf + sizeof(SAVEGAME_PREFIX) - 1, info->pak_extension).version;
		info->OTRP_version = loadsave_t::int_version(buf + sizeof(SAVEGAME_PREFIX) - 1, info->pak_extension).OTRP_version;
		if(  info->version == 0  ) {
			info->version = INVALID_FILE_VERSION;
			return false;
		}
		...
		*s = 0;
		...
		info->version = loadsave_t::int_version(str, info->pak_extension).version;
		info->OTRP_version = loadsave_t::int_version(str, info->pak_extension).OTRP_version;
		if(  info->version == 0  ) {
			info->version = INVALID_FILE_VERSION;
			info->header_size = 0;
			return false;
