...
#ifdef INLINE_OBJ_TYPE
depot_t::depot_t(obj_t::typ type, koord3d pos, player_t *player, const haus_tile_desc_t *t) :
    gebaeude_t(type, pos, player, t)
#else
depot_t::depot_t(koord3d pos, player_t *player, const haus_tile_desc_t *t) :
...
// read/write vehicles in the depot, which are not part of a convoi.
	...
	if(file->is_loading()) {
		...
		if(  this->get_tile()==NULL  ) {
			...
			set_tile( (*hausbauer_t::get_citybuilding_list( haus_desc_t::city_res ))[0]->get_tile(0), true );
		}
