...
int freight_list_sorter_t::compare_ware(const void *td1, const void *td2)
{
  ...
  const travel_details * const td2p = (const travel_details*)td2;
	...
	// first sort according to freight type ...
	const int index = ware1.gib_typ()->gib_index()-ware2.gib_typ()->gib_index();
	if(index!=0) {
		return index;
	}
...
void
freight_list_sorter_t::add_ware_heading( cbuffer_t &buf, uint32 sum, uint32 max, const char *measure, const char *name, const char *what_doing )
{
	// not the first line?
	if(buf.len()>0) {
		buf.append("\n");
	...
	buf.append(translator::translate(measure));
	buf.append(" ");
...
//DBG_MESSAGE("haltestelle_t::get_freight_info()","added %i ware.",pos);
	...
	for (int j = 0; j<pos; j++) {
		...
		if(last_ware_index!=ware.gib_typ()->gib_index()) {
			sint32 sum = 0;
			...
			while(full_list!=NULL) {
				...
				if(last_ware_index<full_iter.get_current().gib_typ()->gib_index()) {
					add_ware_heading( buf, 0, full_iter.get_current().menge, full_iter.get_current().gib_mass(), full_iter.get_current().gib_name(), what_doing );
				}
				else {
					break;
				}
			}
			add_ware_heading( buf, sum, full_list?full_iter.get_current().menge:0, ware.gib_mass(), ware.gib_name(), what_doing );
		}
		// detail amount
		buf.append("   ");
	...
	// still entires left?
	while(full_list  &&  full_iter.next()) {
		add_ware_heading( buf, 0, full_iter.get_current().menge, full_iter.get_current().gib_mass(), full_iter.get_current().gib_name(), what_doing );
	}
}
