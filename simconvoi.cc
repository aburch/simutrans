	...
	if(file->is_saving()) {
		...
		for(  uint8 i=0;  i<anz_vehikel;  i++  ) {
			...
			if(i==0  &&  v->get_besch()->get_waytype()==monorail_wt  &&  v->get_typ()==obj_t::rail_vehicle) {
				...
				vehicle_t *v_neu = new monorail_vehicle_t( v->get_pos(), v->get_besch(), v->get_owner(), NULL );
				v->discard_cargo();
				delete v;
				v = v_neu;
			}
...
station_tile_search_ready: ;
	...
	for(unsigned i=0; i<vehicles_loading; i++) {
		...
		if(  amount  ) {
			time = max( time, (amount*v->get_besch()->get_loading_time()) / max(v->get_cargo_max(), 1) );
			v->calc_image();
...
/**
* set line
* since convoys must operate on a copy of the route's fahrplan (schedule), we apply a fresh copy
* @author hsiegeln
*/
...
/**
* unset line
* removes convoy from route without destroying its fahrplan (schedule)
...
const char* convoi_t::send_to_depot(bool local)
{
	...
	FOR(slist_tpl<depot_t*>, const depot, depot_t::get_depot_list()) {
		if (depot->get_waytype() != v->get_besch()->get_waytype()  ||  depot->get_owner() != get_owner()) {
			continue;
		}
