...
bool can_depart(convoihandle_t cnv, halthandle_t halt, uint32 arrived_time, uint32 time_to_load, bool &coupling_cond, uint32 &go_on_ticks) {
	...
	if(  current_entry.get_wait_for_time()  ) {
		...
		sint64 slot = ((sint64)arrived_time - (sint64)delay_tolerance - (sint64)spacing_shift + (sint64)time_to_load ) * (sint64)current_entry.spacing / (sint64)world()->ticks_per_world_month + (sint64)1;
		while( slot<0 ) {
			slot += current_entry.spacing;
		}
