...
bool two_click_tool_t::is_work_here_alters_map(player_t *player, koord3d pos )
{
	if(  !is_first_click()  ) {
		return false;
	}
	...
	DBG_MESSAGE("two_click_tool_t::is_work_here_alters_map", "Position %s valid=%d", pos.get_str(), value );
	if(  value == 0  ) {
