...
static void show_times(karte_t *welt, karte_ansicht_t *view)
{
	...
	DBG_MESSAGE("test", "display_img(): %i iterations took %i ms", i, get_current_time_millis()-ms );fflush(NULL);

	ms=get_current_time_millis();
	for(i=0;  i<300000;  i++ )
		display_color_img( 2000, 120, 100, 16, 1, 1 );
	DBG_MESSAGE("test", "display_color_img(): %i iterations took %i ms", i, get_current_time_millis()-ms );fflush(NULL);

	ms=get_current_time_millis();
	for(i=0;  i<300000;  i++ )
		display_color_img( 2000, 120, 100, 20, 1, 1 );
	DBG_MESSAGE("test", "display_color_img(), other AI: %i iterations took %i ms", i, get_current_time_millis()-ms );fflush(NULL);

	ms=get_current_time_millis();
	for(i=0;  i<300;  i++ )
		display_flush_buffer();
	DBG_MESSAGE("test", "display_flush_buffer(): %i iterations took %i ms", i, get_current_time_millis()-ms );fflush(NULL);
...
static void zeige_banner()
{
	...
	for(s=1; s>=0; s--) {
		...
		display_proportional(xoff+s+24,yoff+s+16, "This is a beta version of Simutrans:", ALIGN_LEFT, color, true);

		if(s == 0) color = COL_WHITE;
		display_proportional(xoff+s+48,yoff+s+28, "Version " VERSION_NUMBER " " VERSION_DATE, ALIGN_LEFT, color, true);

		if(s == 0) color = heading;
		display_proportional(xoff+s+24,yoff+s+48, "This version is developed by", ALIGN_LEFT, color, true);
		...
		if(s == 0) color = heading;
		display_proportional(xoff+s+24,yoff+s+122, "Please send ideas and questions to:", ALIGN_LEFT, color, true);
		...
		if(s == 0) color = heading;
		display_proportional(xoff+s+24,yoff+s+158+12, "or visit the Simutrans pages on the web:", ALIGN_LEFT, color, true);
	...
	do {
		...
		if(!umgebung_t::testlauf && loadgame == "") {
			...
			do {
				...
				check_pos_win(&ev);
				simusleep(10);
			} while(! (wg->gib_load() ||  wg->gib_load_heightfield() ||  wg->gib_start() ||  wg->gib_close() ||  wg->gib_quit()) );
