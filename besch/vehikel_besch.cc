...
{
	uint16 month_now = welt->get_current_month();
	uint32 normal_fixed_costs = get_fixed_maintenance();
	if(!welt->use_timeline() || !is_retired(month_now) || normal_fixed_costs == 0)	
	{
		return normal_fixed_costs;
	}
	//Else
	uint32 base_obsolete_fixed_costs = normal_fixed_costs * (welt->get_einstellungen()->get_obsolete_running_cost_increase_percent() / 100.0F);
	uint16 obsolete_date = get_retire_year_month();
	if((month_now - obsolete_date) >= (welt->get_einstellungen()->get_obsolete_running_cost_increase_phase_years() * 12.0F))
	{
