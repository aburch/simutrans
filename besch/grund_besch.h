...
public:
	...
	{
		bildliste_besch_t const* const liste = get_child<bildliste2d_besch_t>(2)->get_liste(typ);
		if(liste && liste->get_anzahl() > 0) {
			bild_besch_t const* const bild = get_child<bildliste2d_besch_t>(2)->get_bild(typ,stage);
			return bild;
		}
		return NULL;
	}
