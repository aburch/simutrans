...
void path_explorer_t::compartment_t::step()
{
	...
	switch (current_phase)
	{
		...
		case phase_rebuild_connexions :
		{
			...
			const uint16 journey_time_adjustment = world->get_einstellungen()->get_distance_per_tile() * 6;
			...
			while (phase_counter < linkages_count)
			{
				...
				for (uint8 i = 0; i < entry_count; ++i)
				{
					...
					(
						(uint16)( (accurate_distance( halt_list[i]->get_basis_pos(), 
															halt_list[(i+1)%entry_count]->get_basis_pos() ) * journey_time_factor) / 100 ),
						64 
					);
