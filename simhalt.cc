...
void haltestelle_t::get_freight_info(cbuffer_t & buf)
{
	if (resort_freight_info)
	{
		...
		for (unsigned i = 0; i < goods_manager_t::get_max_catg_index(); i++)
		{
			...
			if (warray)
			{
				freight_list_sorter_t::sort_freight(*warray, buf, (freight_list_sorter_t::sort_mode_t)sortierung, NULL, "waiting", NULL, NULL);
			}
		}
		...
		// show new info
		freight_list_sorter_t::sort_freight(ware_transfers, buf, (freight_list_sorter_t::sort_mode_t)sortierung, NULL, "transferring", NULL, NULL);
	}
}
