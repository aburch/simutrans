...
//	const uint32 max_small_city_size = sets->get_max_small_city_size();
	...
	if(city_population.get_count() < city_population_target_count)
	{
		...
		while (city_population.get_count() < city_population_target_count) {
			const double z = (double)simrand_plain() / UINT32_MAX_VALUE;	// z in [0, 1]
...
bool karte_t::rem_fab(fabrik_t *fab)
{
	...
	FOR (vector_tpl<koord>, const k, tile_list) {
		...
		if(plan)
		{
			...
			for(  uint8 i = 0;  i < count;  i++  ) {
				...
				plan->remove_from_haltlist( tmp_list[i].halt );
			}
		}
	}
	...
	koord pos = fab->get_pos().get_2d();
 	const planquadrat_t* plan = access(pos);
	if (plan) {
		...
		*/
		...
		for(int trip_count = 0; trip_count < onward_trips && route_status != no_route && route_status != too_slow && route_status != overcrowded && route_status != destination_unavailable; trip_count ++)
		{
			...
			case private_car:
				...
				city->erzeuge_verkehrsteilnehmer(origin_pos.get_2d(), car_minutes, destination_pos, pax_left_to_do);
#endif
				set_return_trip = true;
				// We cannot do this on arrival, as the ware packets do not remember their origin building.
				if(trip == commuting_trip)
			...
			if(set_return_trip)
			{
				...
				if(return_in_private_car)
				{
					if(car_minutes < 65535)
					{
						...
						city->erzeuge_verkehrsteilnehmer(first_destination.location, car_minutes, origin_pos.get_2d(), pax_left_to_do);
#endif
