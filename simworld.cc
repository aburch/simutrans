...
void karte_t::create_rivers( sint16 number )
{
	...
	while (number > 0 && !mountain_tiles.empty() && retrys++ < 100) {
		...
		sint16 dist = shortest_distance(start,end);
...
#	define REVISION 0
			...
			buf.printf( "&ver=Simutrans %s (r%s) built %s", QUOTEME(VERSION_NUMBER, NARROW_EXPERIMENTAL_VERSION), QUOTEME(REVISION), QUOTEME(VERSION_DATE) );
			// Pakset version
			buf.append( "&pak=" );
