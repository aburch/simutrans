...
const char *brueckenbauer_t::baue( spieler_t *sp, const koord3d pos, const bruecke_besch_t *besch)
{
	...
	const char *msg;
	sint8 bridge_height;
	koord3d end = finde_ende(sp, gr->get_pos(), zv, besch, msg, bridge_height );
...
DBG_MESSAGE("brueckenbauer_t::baue()", "end not ok");
	...
	if(besch->get_waytype()==powerline_wt) {
		baue_bruecke(sp, gr->get_pos(), end, zv, bridge_height, besch, lt->get_besch() );
	}
	else {
		baue_bruecke(sp, gr->get_pos(), end, zv, bridge_height, besch, weg->get_besch() );
	}
...
void brueckenbauer_t::baue_bruecke(spieler_t *sp, const koord3d start, const koord3d end, koord zv, sint8 bridge_height, const bruecke_besch_t *besch, const weg_besch_t *weg_besch)
{
	...
	if (slope || bridge_height != 0) {
		...
		baue_auffahrt( sp, start, ribi, slope?0:hang_typ(zv)*add_height, besch, weg_besch );
		if(  besch->get_waytype() != powerline_wt  ) {
...
//DBG_MESSAGE("bool brueckenbauer_t::baue_bruecke()","h1=%i, h2=%i",pos.z,gr->get_pos().z);
	...
	if(  need_auffahrt  ) {
		...
		baue_auffahrt(sp, end, ribi_typ(-zv), gr->get_weg_hang()?0:hang_typ(-zv)*(pos.z-end.z), besch, weg_besch);
	}
	else {
...
void brueckenbauer_t::baue_auffahrt(spieler_t* sp, koord3d end, ribi_t::ribi ribi_neu, hang_t::typ weg_hang, const bruecke_besch_t* besch, const weg_besch_t *weg_besch)
{
	assert(weg_hang >= 0);
	assert(weg_hang < 81);
