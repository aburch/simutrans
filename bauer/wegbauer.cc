...
bool
wegbauer_t::ist_grund_fuer_strasse(koord pos, koord start, koord ziel) const
{
    bool ok = false;
	...
	case strasse:
	    ok =
		(ok || bd->gib_weg(weg_t::strasse)) &&
		!bd->hat_gebaeude(hausbauer_t::frachthof_besch) &&
		!bd->gib_depot();
	    break;
	case schiene:
	    ok = (ok || bd->gib_weg(weg_t::schiene)) &&
	...
	case schiene_bot_bau:
	    ok = ok || (pos==start || pos==ziel) && bd->gib_weg(weg_t::schiene) != NULL && bd->gib_besitzer() == sp;
	    break;
	default:
	    break;
	}
...
wegbauer_t::intern_calc_route(const koord start, const koord ziel)
{
    ...
    if(!ist_grund_fuer_strasse(ziel, start, ziel)) {
	return;
    }
    // Verginder, das wir eine Strasse der L#nge 1 auf einem
    // schiefn Hang bauen:
...
//	koord force_dir = koord(0, 0);
	...
	for(int r=0; r<4; r++) {
		...
		// allgemeine checks, untergrund
                if(!ist_grund_fuer_strasse(t, start, ziel)) {
		    continue;
		}

		// berechne kosten für den Schritt

		const int cost = calc_cost(t);
...
wegbauer_t::baue_tunnel_und_bruecken()
{
	...
	if(d.x > 1 || d.y > 1 || d.x < -1 || d.y < -1) {
		...
		if(bautyp == strasse) {
		    brueckenbauer_t::baue(sp, welt, route->at(i), weg_t::strasse);
		} else if(bautyp == schiene || bautyp == schiene_bot_bau){
		    brueckenbauer_t::baue(sp, welt, route->at(i), weg_t::schiene);
	        }
	    }
	}
    }
