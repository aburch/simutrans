...
public:
	 ...
	 */
	bool insert(const grund_t* gr, uint8 minimum_loading = 0, uint16 waiting_time_shift = 0, uint16 stop_flags = 0, uint16 length_coupling_done = 0);

	/**
	 * Appends a coordinate to the schedule.
	 */
	bool append(const grund_t* gr, uint8 minimum_loading = 0, uint16 waiting_time_shift = 0, uint16 stop_flags = 0, uint16 length_coupling_done = 0);
