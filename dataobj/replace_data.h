...
public:
	...
	const vector_tpl<const vehikel_desc_t *>* get_replacing_vehicles() const { return replacing_vehicles; }
	const vehikel_desc_t* get_replacing_vehicle(uint16 number) const;
	void set_replacing_vehicles(vector_tpl<const vehikel_desc_t *> *rv) { replacing_vehicles = rv; }
	void add_vehicle(const vehikel_desc_t* vehicle, bool add_at_front = false);
