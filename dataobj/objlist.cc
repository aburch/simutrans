...
//		DBG_MESSAGE("objlist_t::dump()","empty");
		...
		DBG_MESSAGE("objlist_t::dump()","one object \'%s\' owned by player %p", obj.one->get_name(), obj.one->get_besitzer() );
		return;
	}

	DBG_MESSAGE("objlist_t::dump()","%i objects", top );
	for(uint8 n=0; n<top; n++) {
		DBG_MESSAGE( obj.some[n]->get_name(), "at %i owned by player %p", n, obj.some[n]->get_besitzer() );
	}
}
