...
schedule_entry_t schedule_t::dummy_entry(koord3d::invalid, 0, 0, 0, 0, 0);
...
bool schedule_t::insert(const grund_t* gr, uint8 minimum_loading, uint16 waiting_time_shift, uint16 stop_flags, uint16 max_speed_kmh_of_convoi, uint16 length_coupling_done)
{
	...
	if(  is_stop_allowed(gr)  ) {
		entries.insert_at(current_stop, schedule_entry_t(gr->get_pos(), minimum_loading, waiting_time_shift, stop_flags, max_speed_kmh_of_convoi, length_coupling_done));
		current_stop ++;
...
bool schedule_t::append(const grund_t* gr, uint8 minimum_loading, uint16 waiting_time_shift, uint16 stop_flags, uint16 max_speed_kmh_of_convoi, uint16 length_coupling_done)
{
	// stored in minivec, so we have to avoid adding too many
	if(entries.get_count()>=254) {
		create_win( new news_img("Maximum 254 stops\nin a schedule!\n"), w_time_delete, magic_none);
	...
	if(is_stop_allowed(gr)) {
		if( next_line.is_bound() ) {
			entries.insert_at(entries.get_count()-1, schedule_entry_t(gr->get_pos(), minimum_loading, waiting_time_shift, stop_flags, max_speed_kmh_of_convoi, length_coupling_done));
			return true;
		} else {
			entries.append(schedule_entry_t(gr->get_pos(), minimum_loading, waiting_time_shift, stop_flags, max_speed_kmh_of_convoi, length_coupling_done), 4);
			return true;
		}
	}
	else {
...
void schedule_t::rdwr(loadsave_t *file)
{
	...
	if(file->is_version_less(99, 12)) {
		for(  uint8 i=0; i<size; i++  ) {
			...
			pos.rdwr(file);
			file->rdwr_long(dummy);
			entries.append(schedule_entry_t(pos, (uint8)dummy, 0, 0, 0, 0));
		}
	}
	else {
...
void schedule_t::sprintf_schedule( cbuffer_t &buf ) const
{
	...
	FOR(minivec_tpl<schedule_entry_t>, const& i, entries) {
		buf.printf("%s,%i,%i,%i,%i,%i,%i,%i,%i|", i.pos.get_str(), (int)i.minimum_loading, (int)i.waiting_time_shift, i.get_stop_flags(), i.max_speed_kmh_of_convoi, i.spacing, i.spacing_shift, i.delay_tolerance, i.length_coupling_done);
	}
}
