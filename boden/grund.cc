	...
	if(neighbouring_grounds.get_count() > 1)
	{
		...
		FOR(minivec_tpl<grund_t*>, const& gr, neighbouring_grounds)
		{
			...
			if(end != koord3d::invalid)
			{
				...
				const bool route_good = !way_gr->ist_bruecke() && diversionary_route.calc_route(welt, start, end, diversion_checker, max_speed, max_axle_load, false, 0, welt->get_settings().get_max_diversion_tiles() * 100, bridge_weight_limit, false, w->get_pos());
