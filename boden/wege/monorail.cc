...
void monorail_t::rdwr(loadsave_t *file)
{
	...
	if(get_besch()->get_wtyp()!=monorail_wt) {
		...
		const weg_besch_t *besch = wegbauer_t::weg_search( monorail_wt, (old_max_speed>0 ? old_max_speed : 120), (old_max_axle_load > 0 ? old_max_axle_load : 10), 0, (systemtype_t)((get_besch()->get_styp()==type_elevated)*type_elevated), 1);
		if (besch==NULL) {
