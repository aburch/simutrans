...
const weg_desc_t *monorail_t::default_monorail=NULL;
...
void monorail_t::rdwr(loadsave_t *file)
{
	...
	if(get_desc()->get_wtyp()!=monorail_wt) {
		...
		const weg_desc_t *desc = wegbauer_t::weg_search( monorail_wt, (old_max_speed>0 ? old_max_speed : 120), (old_max_axle_load > 0 ? old_max_axle_load : 10), 0, (systemtype_t)((get_desc()->get_styp()==type_elevated)*type_elevated), 1);
		if (desc==NULL) {
