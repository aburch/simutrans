		...
		case convoi_t::OUT_OF_RANGE:
			{
				...
				DBG_MESSAGE("player_t::report_vehicle_problem","Vehicle %s cannot travel %f.2km to (%i,%i) because it would exceed its range of %i by %.2fkm", cnv->get_name(), distance, ziel.x, ziel.y, cnv->get_min_range(), excess);
				if(this == welt->get_active_player())
				{
					cbuffer_t buf;
					...
					buf.printf( translator::translate("Vehicle %s cannot travel %f.2km to %s because that would exceed its range of %ikm by %.2fkm"), cnv->get_name(), distance, name, cnv->get_min_range(), excess);
					welt->get_message()->add_message( (const char *)buf, cnv->get_pos().get_2d(), message_t::warnings, PLAYER_FLAG | player_nr, cnv->front()->get_base_image());
				}
			}
			break;
