...
halthandle_t ai_passenger_t::build_airport(const stadt_t* city, koord pos, int rotation)
{
	...
	bauigel.route_fuer(wegbauer_t::strasse, wegbauer_t::weg_search(road_wt, road_vehicle->get_geschw(), road_vehicle->get_gewicht(), welt->get_timeline_year_month(), type_flat, 1), tunnelbauer_t::find_tunnel(road_wt,road_vehicle->get_geschw(),welt->get_timeline_year_month()), brueckenbauer_t::find_bridge(road_wt,road_vehicle->get_geschw(),welt->get_timeline_year_month()) );
	bauigel.set_keep_existing_faster_ways(true);
	bauigel.set_keep_city_roads(true);
	bauigel.set_maximum(10000);
