...
bool ai_passenger_t::create_water_transport_vehikel(const stadt_t* start_stadt, const koord target_pos)
{
	...
	if(!start_hub.is_bound()) {
		...
		if(town_road!=bushalt) {
			...
			bauigel.route_fuer( wegbauer_t::strasse, wegbauer_t::weg_search( road_wt, 25, welt->get_timeline_year_month(), type_flat ), tunnelbauer_t::find_tunnel(road_wt,road_vehicle->get_geschw(),welt->get_timeline_year_month()), NULL );
	...
	if(!start_hub.is_bound()) {
		...
		const haus_besch_t* dock_desc = hausbauer_t::get_random_station(haus_besch_t::dock, water_wt, welt->get_timeline_year_month(), 0);
				...
				if(line->get_finance_history(0,LINE_PROFIT)<0) {
					...
					if(welt->use_timeline()  &&  line->count_convoys()>1) {
						...
						if(capacity>0) {
							...
							vehikel_besch_t const* const  v_desc = vehikelbauer_t::vehikel_search(wt, welt->get_current_month(), 50, welt->get_average_speed(wt), warenbauer_t::passagiere, false, true);
