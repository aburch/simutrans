...
bool ai_goods_t::create_ship_transport_vehikel(fabrik_t *qfab, int anz_vehikel)
{
	...
	halthandle_t halt = haltestelle_t::get_stoppable_halt(gr->get_pos(),this);
			...
			if(ship_vehicle) {
				...
				if(  start_halt.is_bound()  &&  (start_halt->get_station_type()&haltestelle_t::dock)!=0  ) {
					...
					if(!lines.empty()) {
						...
						if(schedule->get_count()>1  &&  haltestelle_t::get_stoppable_halt(schedule->at(0).pos,this)==start_halt) {
			...
			for (size_t i = welt->convoys().get_count(); i-- != 0;) {
				...
				if(delete_this) {
					...
					if(  !line.is_bound()  ||  line->count_convoys()==0  ) {
						...
						halthandle_t start_halt = haltestelle_t::get_stoppable_halt(start_pos,this);
						if(start_halt.is_bound()  &&  (start_halt->get_station_type()&haltestelle_t::dock)!=0) {
							// delete all ships on this line
							vector_tpl<linehandle_t> lines;
