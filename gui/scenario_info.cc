...
void scenario_info_t::resize(const koord delta)
{
	...
	gui_flowtext_t *texts[] = { &info, &goal, &rule, &result, &about, &error, &debug_msg};
	koord gr = get_client_windowsize() - info.get_pos() - koord(D_MARGIN_RIGHT + D_SCROLLBAR_WIDTH, D_MARGIN_BOTTOM + D_SCROLLBAR_HEIGHT);
	for(uint32 i=0; i<lengthof(texts); i++) {
...
void scenario_info_t::update_scenario_texts(bool init)
{
	...
	koord border_size = get_client_windowsize() - info.get_pos() - koord(D_MARGIN_RIGHT + D_SCROLLBAR_WIDTH, D_MARGIN_BOTTOM + D_SCROLLBAR_HEIGHT);
	if (init) {
