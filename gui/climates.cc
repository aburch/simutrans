	...
	gui_frame_t( translator::translate("Climate Control") )
{
	...
	// Water level
	water_level.init( sets->get_grundwasser(), -10*(grund_desc_t::double_grounds?2:1), 0, gui_numberinput_t::AUTOLINEAR, false );
	...
	for(  int i=desert_climate-1;  i<=rocky_climate-1;  i++  ) {
		...
		numberinput_lbl[labelnr].init( grund_desc_t::get_climate_name_from_bit((climate)(i+1)), cursor + lbl_offs);
		numberinput_lbl[labelnr].set_width(label_width);
