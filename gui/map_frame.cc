...
map_button_t button_init[MAP_MAX_BUTTONS] = {
	...
	{ COL_LIGHT_YELLOW, COL_DARK_YELLOW,       "Passagiere", "Show passenger coverage", reliefkarte_t::MAP_PASSENGER },
	{ COL_LIGHT_YELLOW, COL_DARK_YELLOW,       "Post", "Show mail service coverage", reliefkarte_t::MAP_MAIL },
	{ COL_LIGHT_YELLOW, COL_DARK_YELLOW,       "Fracht", "Show transported freight", reliefkarte_t::MAP_FREIGHT },
	...
	max_label("max")
{
	...
	for(  int np = 0, count = 1;  np < MAX_PLAYER_COUNT;  np++  ) {
		if(  welt->get_spieler( np )  &&  welt->get_spieler( np )->has_money_or_assets()) {
			viewed_player_c.append_element( new gui_scrolled_list_t::const_text_scrollitem_t(welt->get_spieler( np )->get_name(), COL_BLACK));
