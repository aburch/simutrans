	...
	signal_label("signal spacing")
{
	...
	signal_spacing_inp.set_pos( koord( L_DIALOG_WIDTH - D_MARGIN_RIGHT - signal_spacing_inp.get_groesse().x, cursor.y ) );
	...
	signal_label.align_to( &signal_spacing_inp, ALIGN_CENTER_V, koord( cursor.x, 0 ) );
	signal_label.set_width( signal_spacing_inp.get_pos().x - D_MARGIN_LEFT - D_H_SPACE );
	add_komponente( &signal_label );

	cursor.y += signal_spacing_inp.get_groesse().y + D_V_SPACE;
	...
	cursor.y += remove_button.get_groesse().y + D_V_SPACE;
	...
	cursor.y += replace_button.get_groesse().y;
