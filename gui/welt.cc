	...
	map(0,0)
{
	...
	world = world_par;
	sets = sets_par;
	sets->beginner_mode = umgebung_t::default_einstellungen.get_beginner_mode();
...
bool welt_gui_t::action_triggered( gui_action_creator_t *komp,value_t v)
{
	...
	if(komp==&inp_map_number) {
		...
		loaded_heightfield = false;
		sets->heightfield = "";
		sets->grundwasser = -2*umgebung_t::pak_height_conversion_factor;
		...
		if(loaded_heightfield) {
			world->load_heightfield(&umgebung_t::default_einstellungen);
		}
		else {
			umgebung_t::default_einstellungen.heightfield = "";
			world->init( &umgebung_t::default_einstellungen, 0 );
		}
		destroy_all_win(true);
		world->step_month( umgebung_t::default_einstellungen.get_starting_month() );
