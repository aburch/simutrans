 ...
 : infowin_t(welt), buttons(31)
{
    ...
    for (i=0; i<24; i+=2) {
		...
		button_def.setze_pos( koord((i<22)?intLeftOfButton+37:intLeftOfButton+57,intTopOfButton) );
...
void welt_gui_t::infowin_event(const event_t *ev)
{
	...
	}  else if(buttons.at(29).getroffen(ev->mx, ev->my)) {
	    if (IS_LEFTCLICK(ev)) {
		quit = true;
	    }
	} else if(buttons.at(30).getroffen(ev->mx, ev->my)) {
	    if (IS_LEFTCLICK(ev)) {
...
welt_gui_t::gib_fensterbuttons()
{
      ...
      //28-Oct-2001 Markus Weber added button
    buttons.at(24).text = translator::translate("Random map");

    // setze variable anteile der Buttons
    buttons.at(25).pressed = sets->gib_show_pax();
    buttons.at(25).text = translator::translate("7WORLD_CHOOSE"),

    buttons.at(26).pressed = umgebung_t::night_shift;
    buttons.at(26).text = translator::translate("8WORLD_CHOOSE"),


    buttons.at(27).text = translator::translate("Lade Spiel");
    buttons.at(28).text = translator::translate("Starte Spiel");
    buttons.at(29).text = translator::translate("Beenden");
    buttons.at(30).text = translator::translate("Lade Relief");
...
void welt_gui_t::zeichnen(koord pos, koord gr)
{
  ...
  display_ddd_box_clip(x+10, y+295, 230, 0, MN_GREY0, MN_GREY4);
