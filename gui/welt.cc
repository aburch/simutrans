...
welt_gui_t::update_preview()
{
	...
	for(int j=0; j<preview_size; j++) {
		for(int i=0; i<preview_size; i++) {
			karte[j*preview_size+i] = reliefkarte_t::calc_hoehe_farbe((karte_t::perlin_hoehe(i*mx, j*my, sets->gib_map_roughness(), sets->gib_max_mountain_height())>>4)+1, sets->gib_grundwasser()>>4);
		}
	}
	sets->heightfield = "";
