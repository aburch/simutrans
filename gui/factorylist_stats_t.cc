		...
		if(fab) {
			...
			if (display_operation_stats) {
				...
				if (!fab->get_input().empty())
				{
					...
					display_linear_gradiwnt_wh(xoff + offset_left + 14 + 32 + 12, yoff + 2, colored_with, LINESPACE - 3, colored_with == STORAGE_BAR_WIDTH ? COL_DARK_GREEN : COL_DARK_GREEN + 1, 10, 75, false);
				...
				if (!fab->get_output().empty()) {
					...
					if (fab->get_total_output_capacity()) {
						...
						display_linear_gradiwnt_wh(xoff + offset_left + 12, yoff + 2, colored_with, LINESPACE - 3, colored_with == STORAGE_BAR_WIDTH ? COL_DARK_GREEN : COL_DARK_GREEN + 1, 10, 75, false);
					}
