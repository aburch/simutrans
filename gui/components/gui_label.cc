...
void gui_label_updown_t::draw(scr_coord offset)
{
	...
	if (align == money_right) {
		...
		display_fluctuation_triangle_rgb(right.x - LINEASCENT + 3, right.y + LINEASCENT/6, LINEASCENT*2/3, true, value);
	}
	else {
		display_fluctuation_triangle_rgb(pos.x + offset.x, pos.y + offset.y + LINEASCENT/6, LINEASCENT*2/3, true, value);
