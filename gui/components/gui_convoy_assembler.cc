...
void gui_convoy_assembler_t::update_data()
{
	...
	if(vehicles.get_count() > 0) {
		...
		convoi_pics[0]->basic_coupling_constraint = vehicles[0]->get_coupling_constraint();
	...
	FOR(vehicle_image_map, const& i, vehicle_map) 
	{
		...
		img.lcolor = ok_color;
		img.rcolor = ok_color;
		img.basic_coupling_constraint = info->get_coupling_constraint();
...
//	left += D_MARGIN_LEFT;
//	//display_proportional(left, top, translator::translate("Color"), ALIGN_LEFT, SYSCOL_TEXT, true);
	int padding = 0;
...
//		padding = max(padding, display_proportional(left + VEHICLE_BAR_HEIGHT * 2 + 3, top, translator::translate(bar_color_helptexts[i]), ALIGN_LEFT, CITY_KI, true));
//
//		//;
//
//		top += LINESPACE;
...
//	left += padding + VEHICLE_BAR_HEIGHT * 2 + 3 + D_H_SPACE;
	...
	left += D_MARGIN_LEFT;
	display_veh_form(left, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 5, 0, false, false);
	display_veh_form(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, vehicle_desc_t::can_be_tail_next, 0, true, false);
	padding = display_proportional(left + VEHICLE_BAR_HEIGHT*4 + 3, top, translator::translate("helptxt_one_direction"), ALIGN_LEFT, CITY_KI, true);

	top += LINESPACE;
	display_veh_form(left, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 15, 1, false, false);
	display_veh_form(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 15, 1, true, false);
	padding = max(padding, display_proportional(left + VEHICLE_BAR_HEIGHT*4 + 3, top, translator::translate("helptxt_cab_head"), ALIGN_LEFT, CITY_KI, true));

	top += LINESPACE;
	display_veh_form(left, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 12, 1, false, false);
	display_veh_form(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 12, 1, true, false);
	padding = max(padding, display_proportional(left + VEHICLE_BAR_HEIGHT*4 + 3, top, translator::translate("helptxt_can_be_at_rear"), ALIGN_LEFT, CITY_KI, true));

	top += LINESPACE;
	display_veh_form(left, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 0, 0, false, false);
	display_veh_form(left + VEHICLE_BAR_HEIGHT*2, top + 2, VEHICLE_BAR_HEIGHT*2, COL_DARK_GREEN, true, 0, 0, true, false);
	...
	padding = max(padding, display_proportional(left + VEHICLE_BAR_HEIGHT*4 + 3, top, translator::translate("helptxt_fixed_outside_depot"), ALIGN_LEFT, CITY_KI, true));
	...
	display_proportional(left + VEHICLE_BAR_HEIGHT*4 + 3, top, translator::translate("helptxt_permanent_couple"), ALIGN_LEFT, CITY_KI, true);
