...
void gui_chart_t::draw(scr_coord offset)
{
	...
	if(background != -1) {
		display_fillbox_wh_clip(offset.x, offset.y, chart_size.w, chart_size.h, background, false);
	}
	int tmpx, factor;
	...
	// draw zero line
	display_direct_line(offset.x+1, offset.y+(scr_coord_val)baseline, offset.x+chart_size.w-2, offset.y+(scr_coord_val)baseline, SYSCOL_CHART_LINES_ZERO);
	...
	if (show_y_axis) {
		...
		display_proportional_clip(offset.x - 4, offset.y+chart_size.h-5, cmin, ALIGN_RIGHT, SYSCOL_TEXT_HIGHLIGHT, true );
	}

	// draw chart frame
	display_ddd_box_clip(offset.x, offset.y, chart_size.w, chart_size.h, SYSCOL_SHADOW, SYSCOL_HIGHLIGHT);
	...
	for(  int i = 0;  i < x_elements;  i++  ) {
		...
		display_vline_wh_clip( x0, offset.y + 1, chart_size.h - 2, line_color, false );
	}
	...
	FOR(slist_tpl<curve_t>, const& c, curves) {
		if (c.show) {
			...
			for (int i=0;i<c.elements;i++) {
				...
				display_fillbox_wh_clip(tmpx+factor*(chart_size.w / (x_elements - 1))*i-2, (scr_coord_val)(offset.y+baseline- (int)(tmp/scale)-2), 5, 5, c.color, true);
				...
				if (i>0) {
					display_direct_line(tmpx+factor*(chart_size.w / (x_elements - 1))*(i-1),
					...
					if(  c.show_value  ) {
						if(  env_t::left_to_right_graphs  ) {
							number_to_string_fit(cmin, (double)tmp, c.precision, maximum_axis_len-c.type );
	...
	FOR(slist_tpl<line_t>, const& line, lines) {
		if(  line.show  ) {
			...
			for(  int t=0;  t<line.times;  ++t  ) {
				// display marker(box) for financial value
				display_fillbox_wh_clip(tmpx+factor*(chart_size.w / (x_elements - 1))*t-2, (scr_coord_val)(offset.y+baseline- (int)(tmp/scale)-2), 5, 5, line.color, true);
			...
			display_fillbox_wh(tmpx, y_offset, factor*(chart_size.w / (x_elements - 1))*(line.times-1), 1, line.color, true);
		}
	}
}
