...
void schedule_gui_t::extract_advanced_settings(bool yesno) {
	...
	const bool reversable_waytype = env_t::reversable_waytype(schedule->get_waytype());
	bt_wait_for_child.set_visible(coupling_waytype  &&  yesno);
	bt_find_parent.set_visible(coupling_waytype  &&  yesno);
	bt_reverse_convoy.set_visible(reversable_waytype  &&  yesno);
	bt_reverse_coupling.set_visible(reversable_waytype  &&  yesno);
