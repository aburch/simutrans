...
void city_info_t::update_labels()
{
	...
	double urban_density = 1.0 * max(0.0, (c->get_einwohner() - (c->get_unemployed() + c->get_homeless())/2.0)) / max(c->get_buildings()*1.0,1.0);
	lb_urban_density.buf().printf("%s: %.2f /%s", translator::translate("Urban Density"),urban_density, translator::translate("building"));	lb_urban_density.update();
