...
car_group_t::car_group_t(karte_t * w, koord3d pos)
{
  ...
  for(i=0; i<num_cars; i++) {
    ...
    if(i == 0) {
      vb = vehikelbauer_t::vehikel_fuer_leistung(2060,
						 vehikel_besch_t::schiene, 0);
    } else {
...
void car_group_t::start()
{
  ...
  if(ok) {
    ...
    DBG_MESSAGE("car_group_t::start()", "route found");
  } else {
    ...
    DBG_MESSAGE("car_group_t::start()", "no route found, using first possible direction");
  }


  DBG_MESSAGE("car_group_t::start()", "pos=%d,%d,%d  next=%d,%d,%d", pos.x, pos.y, pos.z, next.x, next.y, next.z);
   int i;
  for(i=0; i<num_cars; i++) {
    cars[i]->setze_pos(pos);
...
void car_group_t::do_driving()
{
  ...
  if(cars[0]->is_about_to_hop()) {
	...
	advance_schedule();
      ...
      DBG_MESSAGE("car_group_t::sync_step()", "Hop detected, guess next field %d,%d,%d", dest.x, dest.y, dest.z);
  ...
  if(cars[0]->is_about_to_hop()) {
	...
	assert(!cars[0]->is_about_to_hop());
      ...
      */
      ...
      DBG_MESSAGE("car_group_t::do_enter_station()", "Changing to LOADING at %d,%d,%d", tmp.x, tmp.y, tmp.z);
      return;
    }
  }
...
koord3d car_group_t::calc_next_pos()
{
  ...
  if(num_nb == 0) {
    ...
    DBG_MESSAGE("car_group_t::direction()", "Dead end, turning.");
    ...
    DBG_MESSAGE("car_group_t::direction()",
...
koord3d car_group_t::guess_direction(grund_t **nb, int num_nb)
{
  ...
  if(ok) {
    ...
    DBG_MESSAGE("car_group_t::guess_direction()", "got unblocked (shortest) path 1");

    if(!is_blocked(pos, dest)) {
      // Hajo: Got a unblocked shortes path -> use it if we don't need to turn!

      DBG_MESSAGE("car_group_t::guess_direction()", "got unblocked (shortest) path 2");
  ...
  DBG_MESSAGE("car_group_t::guess_direction()", "try to guess");
